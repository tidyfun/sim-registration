...
than the oracle pipeline — see @sec-fda-default-negative for interpretation.
...
## fda_default: Negative Oracle Benefit {#sec-fda-default-negative}

```{r}
#| label: fda-default-negative
#| results: asis
fda_def <- benefit_wide[method == "fda_default"]
...
cat(sprintf(
  "**fda_default** shows negative oracle benefit in %d of %d DGP-condition cells ",
...
cat("2. **Objective mismatch**: fda_default uses criterion 2 (eigenfunction-based), ",
...
# Compare with fda_crit1
crit1_neg <- benefit_wide[method == "fda_crit1", sum(pct_improvement < 0)]
cat(sprintf(
  "By contrast, **fda_crit1** (criterion 1, ISE-based) shows negative oracle benefit ",
  "in only %d of %d cells, suggesting criterion 2 is more sensitive to the ",
  "iteration/co-adaptation effect.\n",
  crit1_neg, nrow(benefit_wide[method == "fda_crit1"])
))
```
...
cat("benefit. For SRVF, harder conditions amplify the oracle benefit as expected. ",
  "For fda_default, the negative oracle benefit persists or worsens, consistent ",
...
# Key insight: fda_default warp vs alignment divergence
fda_warp <- benefit_wide[method == "fda_default",
  .(warp_med = median(pct_improvement, na.rm = TRUE)), by = condition]
fda_ae <- ae_benefit_wide[method == "fda_default",
...
if (any(sign(fda_both$warp_med) != sign(fda_both$ae_med))) {
  cat("**Key insight — fda_default metric divergence**: The warp MISE oracle ",
  ...
  cat(sprintf(
    "**fda_default** shows consistent sign across metrics: warp MISE median %.0f%%/%.0f%% ",
...
cat("- **Negative oracle benefit** (e.g., fda_default) may partly reflect the ",
...
# fda_default finding
fda_def_benefit <- benefit_wide[method == "fda_default",
  .(
    mean_pct = mean(pct_improvement, na.rm = TRUE),
    se_pct = if (.N > 1) sd(pct_improvement, na.rm = TRUE) / sqrt(.N) else 0,
    median_pct = median(pct_improvement, na.rm = TRUE)
...
cat(sprintf(
  "3. **fda_default is condition-dependent**: mean oracle benefit %.0f ± %.0f pp in the easy condition but %.0f ± %.0f pp in the hard condition (medians %.0f%% and %.0f%%). ",
...
cat("6. **Implication for `tf_register()`**: The asymmetry between methods suggests ",
  ...
  "or multi-resolution estimation, and (b) fda_default's criterion 2 benefits from ",
...
cat("7. **Alignment error tells a different story** (@sec-alignment-error): ",
  ...
  "even fda_default. ")
fda_ae_summary <- ae_final[method == "fda_default"]
cat(sprintf(
  "fda_default's alignment error oracle benefit is %.0f ± %.0f pp (positive), while its ",

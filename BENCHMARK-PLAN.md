...
Monte Carlo benchmark comparing 5 registration methods across 13 DGPs with
...
| rank2 | x_i(t) = a_i * m(h_i(t)) + c_i | a_i ~ LogNormal, c_i ~ N(0, 0.3^2) |
...
### Primary
- **Warp MISE**: `mean(tf_integrate((h^{-1}_est - h^{-1}_true)^2))`
  For domain-preserving warps, inverse warps are compared directly (single
  inversion of smooth truth). For affine warps, forward warps are compared
  (double inversion fallback, but affine warps are simple).
- **Alignment error**: `mean(tf_integrate((aligned_i - true_prewarp_i)^2))`
- **Template MISE**: `tf_integrate((template_est - template_true)^2)`
...
### Study A: Main (26,000 runs)
13 DGPs x 2 severity (0.5, 1.0) x 2 noise (0, 0.1) x 5 methods = 260 cells x 100 reps
...
attic/sim-registration/
  ...
  sim-metrics.R       ~270 lines   All metrics using tf API
  ...
  sim-analyze.R       ~300 lines   Data loading, aggregation, plot helpers
...
- Three warp types via `generate_warps()`:
  ...
  - Affine: TruncNorm with re-centering
...
- `dgp_spec()` dispatches DGP name -> (template, phase, amplitude) tuple
...
6. **Single-inversion for warp metrics**: compare inverse warps directly
   instead of double-inverting estimated warps. For non-DP (affine) warps,
   fall back to forward warp comparison (detected via domain check on
   estimated inverse warps).

**Acknowledged but deferred:**
- Affine recentering can technically push values outside truncation bounds
  (negligible with current severity range)
- Missing `wiggly + simple + none` DGP for factorial balance (not critical)
...
### Phase 6: Study A production run (done)
- 61/65 batches complete (missing D12/D13 for fda_crit1/fda_default)
- Results: 400 rows per batch (100 reps x 2 severity x 2 noise)
- Saved per (DGP, method) as `results_D{nn}_{method}_A.rds`

### Phase 7: Study A Analysis (in progress)
...
### Phases 8+: Sub-studies B, C, D (pending)
...
Study A (26K runs, 61/65 batches complete) compares 5 registration methods across
13 DGPs x 2 severity x 2 noise = 260 cells x 100 reps. The DGP design is NOT full
factorial (13 of 24 possible template x warp x amplitude combos), but it enables clean
...
|--------|------|----------------|
| `warp_mise` | Primary | ISE of estimated vs true warps (lower = better) |
| `alignment_error` | Primary | ISE of aligned curves vs true pre-warp curves |
| `template_mise` | Primary | ISE of estimated vs true template |
| `alignment_cc` | Secondary | Cross-correlation of aligned vs truth (higher = better) |
| `registration_ratio_median` | Diagnostic | `||h-id||^2 / ||h-id||^2` per curve, median. <1 = under-registered, >1 = over-registered |
...
- **Amplitude ladders** (clean comparisons):
  ...
  - wiggly + simple: D06(rank1) -> D11(highrank)
...
- **Warp ladders**:
  - harmonic + none: D01(simple) -> D02(complex) -> D04(affine)
  - harmonic + highrank: D09(simple) -> D10(complex)
  - wiggly + highrank: D11(simple) -> D12(complex) -> D13(affine)
...
- **Metric**: `template_mise` (L^2 MISE with caveat re shape/phase conflation)
...
```
1. Setup
2. Overview: DGP x Method heatmaps
...
6. **Affine sanity**: D04/D13 -- affine_ss should outperform smooth methods
...
- L^2 template MISE conflates shape and phase error -- revisit if suspicious
...
- All code in `attic/sim-registration/`, data in `results/` subdirectory

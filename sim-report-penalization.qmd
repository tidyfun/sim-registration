...
fda_both <- dt[failure == FALSE & method %in% c("fda_default", "fda_crit1")]
...
cor_default <- cor(
  fda_agg_both[method == "fda_default" & severity == "0.5", mise],
  fda_agg_both[method == "fda_default" & severity == "1", mise],
  use = "complete.obs"
)
cor_crit1 <- cor(
  fda_agg_both[method == "fda_crit1" & severity == "0.5", mise],
  fda_agg_both[method == "fda_crit1" & severity == "1", mise],
  use = "complete.obs"
)
cat(sprintf(
  "Results shown for severity=1.0. MISE profiles are highly correlated across severities (r=%.2f for fda_default, r=%.2f for fda_crit1); severity=0.5 patterns are qualitatively similar.\n\n",
...
#| fig-height: 8
#| fig-cap: "Direct comparison of fda_default vs fda_crit1 MISE across lambda values. Points show medians and vertical bars show IQRs in each DGP × noise panel at severity=1.0."
...
fda_comp[, crit1_higher := lambda_fda_crit1 > lambda_fda_default]
fda_comp[, crit1_lower := lambda_fda_crit1 < lambda_fda_default]
fda_comp[, crit1_equal := lambda_fda_crit1 == lambda_fda_default]
...
cat(sprintf(
  "- Paired comparison of optimal λ: fda_crit1 higher in %d/%d, lower in %d/%d, equal in %d/%d cells.\n",
...
fda_comp[, mise_gap := mise_fda_crit1 - mise_fda_default]
